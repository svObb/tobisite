...
# Пороги и потолки товарных секций — копия их контрактов
# (sections/products/products_grid.yaml: requires products_with_images "3..6",
# repeat "3..6"; product_carousel.yaml: ">=7" и repeat "7..10"). Резерв снимков
# под товары считается ровно по ним: расхождение с контрактом ловит
# тест-заслон в test_sections.
#
# Ветки не пересекаются: витрина берёт от трёх товаров с фото до шести, лента —
# от семи. Поэтому резерв точный, а не «максимум из двух потолков».
GRID_MIN = 3
GRID_CAP = 6
CAROUSEL_MIN = 7
CAROUSEL_CAP = 10
...
class Profile:
    ...
    def free_photos(self) -> list[str]:
        ...
        и не встанет в товарную секцию. Товары стейджинг кладёт под теми же
        именами photo-N, и без этого отбора галерея повторяла бы витрину.

        Занятыми считаются только снимки, которые товарная секция реально
        покажет (product_image_names): товар с картинкой за её потолком — и тем
        более витрина, не набравшая товаров на гейт, — снимок не держит.
...
def _product_image_names(p: Profile) -> Feature:
    ...
    Занято ровно то, что способна показать товарная секция: первые GRID_CAP
    товаров с картинкой, а от CAROUSEL_MIN товаров — первые CAROUSEL_CAP,
    потому что дальше витрина уступает роль ленте. Ниже GRID_MIN не проходит
    гейт ни та, ни другая, а products_list картинок не выводит вовсе:
    резервировать нечего, и снимки уходят пулу галереи.

    Допущение осознанное: резерв считает, что товарная секция выигрывает роль
    всегда, когда проходит гейт. Остаток — «гейт пройден, но по score победил
    ...
    cap = CAROUSEL_CAP if len(with_images) >= CAROUSEL_MIN else GRID_CAP
    names = {_image_stem(item["image"].get("src"))
             for item in with_images[:cap]}
