...
DRAFT_TTL_DAYS = 30
# Превью живёт дольше черновика (10.14): ссылку открывают и через месяц после
# письма, а место в R2 стоит копейки. Уборку делает бот, а не lifecycle-правило
# бакета: правило снесло бы и превью проданного лида (10.15).
PREVIEW_TTL_DAYS = 60
