...
IMG_DIR = "img"
# Сколько картинок уезжает в папку превью. Потолок — всё, что способен
# накопить стейджинг по обоим своим контрактам: скрейп кладёт логотип и до
# семи снимков (site_images.MAX_STAGED), амбиент-выкладка доливает поверх фон
# первого экрана и до трёх кадров пула (ambient_stage.AMBIENT_ROLES);
# пересечение имён (hero_bg) уменьшает сумму, а не превышает её. Литералом,
# потому что импорт ambient_stage отсюда — цикл; вывод держит тест в
# tests/test_preview_publish.py. LCP это не трогает: preload берёт только кадры
# первого экрана, остальное грузится лениво.
MEDIA_BUDGET = 12
