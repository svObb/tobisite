...
### 14.6. Учения: сломать релиз намеренно (6.23)

Механизм отката проверяется один раз — на **тестовом** сервере с тест-ботом,
не на бою. Смысл учений: убедиться, что падающий релиз действительно
откатывается сам, а не остаётся лежать.

1. Запомнить текущий тег: `grep '^IMAGE_TAG=' /opt/tobisite-bot/.env`.
2. Отдельной веткой в тестовом репозитории сломать старт так, чтобы падение
   случилось **после** сборки образа: например, добавить в конец `main.py`
   строку `raise SystemExit("учения по откату")`. Ломать импорт нельзя —
   такой коммит не пройдёт CI и до сервера не доедет, а проверяется здесь
   именно сервер.
3. Влить в `main` тестового репозитория и дождаться job `deploy`.
4. Что должно получиться: в логе Actions — `!! не поднялся`, 40 строк лога
   контейнера и `==> откат на <прежний sha>`; job красная.
5. На сервере: `IMAGE_TAG` в `.env` вернулся к прежнему значению,
   `git -C /opt/tobisite-bot rev-parse HEAD` — прежний коммит,
   `docker compose logs --tail 5 bot` показывает `bot started`, тест-бот
   отвечает на `/start`.
6. Убрать поломку следующим коммитом (чинить вперёд, см. 14.5) и убедиться,
   что выкат снова зелёный.

Отдельно стоит проверить вторую ветку отката — неналитую миграцию: добавить
пустую миграцию, не накатывая её, и запустить выкат. Ожидаемое:
`!! есть неналитая миграция — контейнер не тронут`, откат кода и тега,
работающий бот на прежнем образе.
