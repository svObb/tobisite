...
5. **Деплой:** из папки `worker/` → `npx wrangler deploy`
   ...
   `bundle.css` и `fonts/`, собираются на этапе 5.1). Если её ещё нет —
   временно закомментируйте секцию `[assets]` в `wrangler.toml`.
...
- **`/api/hit`** пишет в Analytics Engine только слаг и тип события, без куки
  и без адреса. Нет Analytics Engine на аккаунте — закомментируйте
