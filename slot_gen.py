...
# s5: secondary_label тоже ушёл из free-слотов — подпись второй кнопки берётся
# из заголовка секции, к которой она ведёт (compose знает состав страницы).
# s7: company_label вышел из последней секции, которая его печатала: название
# компании стоит в шапке и в подвале, повторять его в блоке о ней незачем.
PROMPT_VERSION = "s7"
...
side_headline — заголовок второго полотна первого экрана: второе направление
  работы или второй зал компании, не повтор первого заголовка
side_lede — предложение под ним: чем это направление отличается от первого
...
statement — фраза во весь экран поверх фотографии: одно утверждение о работе
  компании. Не лозунг и не обещание — то, что она правда делает
caption — подпись под несколькими фотографиями сразу: что на них снято
